...
fl_cmp_1_train <- select(dat_1[dat_1$run=="train",], epoch, server=loss)
fl_cmp_1_test <- select(dat_1[dat_1$run=="test",], epoch, server=loss)


# split 2
#
dat_2 = read.table("data/federated_2_0.txt",sep=",",quote="\'",header=F)
...
dat_2 <- cbind(epoch=rep(1:(length(dat_2$loss)/4), each=4), dat_2)

fl_cmp_2_train <- select(dat_2[dat_2$run=="train",], epoch, split_2=loss)
fl_cmp_2_train$epoch <- seq(1, length(fl_cmp_2_train$epoch))
fl_cmp_2_test <- select(dat_2[dat_2$run=="test",], epoch, split_2=loss)
fl_cmp_2_test$epoch <- seq(1, length(fl_cmp_2_test$epoch)*3, by=3)

# split 3
#
dat_3 = read.table("data/federated_3_0.txt",sep=",",quote="\'",header=F)
...
# split 4
#
dat_4 = read.table("data/federated_4_0.txt",sep=",",quote="\'",header=F)
...
pcmp1 <- ggplot(flcmp_m, aes(x=epoch, y=value, color=variable))+
  labs(title = "Server and Federated runs, Train loss",
       subtitle = "CIFAR (full and n-split subsets for n agents)",
       caption = "Jan, 2022",
       tag = "Figure 10",
       x = "Epoch",
       y = "Loss",
...
flcmp_m <- na.omit(flcmp_m)

pcmp2 <- ggplot(flcmp_m, aes(x=epoch, y=value, color=variable))+
  labs(title = "Server and Federated runs, Test loss",
       subtitle = "CIFAR (full and n-split subsets for n agents)",
       caption = "Jan, 2022",
       tag = "Figure 11",
       x = "Epoch",
...
grid.arrange(pcmp1, pcmp2, ncol=2, 
             top=textGrob("Train and test loss for a range of splits. Server and Federated runs",
...
fl_cmp_1_train <- select(dat_1[dat_1$run=="train",], epoch, server=accuracy)
fl_cmp_1_test <- select(dat_1[dat_1$run=="test",], epoch, server=accuracy)
...
pcmp1 <- ggplot(flcmp_m, aes(x=epoch, y=value, color=variable))+
  labs(title = "Server and Federated runs, Train accuracy",
       subtitle = "CIFAR (full and n-split subsets for n agents)",
       caption = "Jan, 2022",
       tag = "Figure 12",
       x = "Epoch",
...
pcmp2 <- ggplot(flcmp_m, aes(x=epoch, y=value, color=variable))+
  labs(title = "Server and Federated runs, Test accuracy",
       subtitle = "CIFAR (full and n-split subsets for n agents)",
       caption = "Jan, 2022",
       tag = "Figure 13",
       x = "Epoch",
...
grid.arrange(pcmp1, pcmp2, ncol=2, 
             top=textGrob("Train and test accuracy for a range of splits. Server and Federated runs",
                          gp=gpar(fontsize=20,font=2)))
